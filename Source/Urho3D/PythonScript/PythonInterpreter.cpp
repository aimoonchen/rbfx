...
namespace {
Urho3D::ResourceCache* g_resource_cache = nullptr;
bool file_exist(const ea::string& path)
{
    return g_resource_cache->Exists(path);
}

ea::vector<char> get_file_content(const ea::string& path)
{
    if (!g_resource_cache->Exists(path)) {
        URHO3D_LOGERRORF("File not exist: %s", path.c_str());
        return {};
    }
    auto file = g_resource_cache->GetFile(path, false);
    if (!file) {
...
std::string toCppString(PyObject* pyStr)
{
...
{
    g_resource_cache = Urho3D::Context::GetInstance()->GetSubsystem<Urho3D::ResourceCache>();
    Initialize();
}
...
void PythonInterpreter::RegisterModule(const char* name, PyObject* cmodulem)
{
    PyDict_SetItemString(module_dict_, name, cmodulem);
}
...
int PythonInterpreter::RunSimpleString(const ea::string& code)
{
    return PyRun_SimpleString(code.c_str());
}
PyObject* PythonInterpreter::RunString(const ea::string& code)
{
    if (!initialized_)
    {
        return nullptr;
...
PyObject* PythonInterpreter::CallFunction(const ea::string& moduleName, const ea::string& funcName, PyObject* args)
{
    if (!initialized_)
    {
        return nullptr;
...
bool PythonInterpreter::ExecuteFile(const ea::string& filename)
{
...
bool PythonInterpreter::ExecuteString(const ea::string& string)
{
    return false;
}

bool PythonInterpreter::LoadRawFile(const ea::string& fileName)
{
    return false;
}

bool PythonInterpreter::ExecuteRawFile(const ea::string& fileName)
{
    return false;
}
bool PythonInterpreter::ExecuteFunction(const ea::string& functionName)
{
    return false;
}
