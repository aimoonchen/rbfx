...
{
    bxColor rgb;
    rgb.x_ = M_XYZ2RGB[0] * xyz.x_ + M_XYZ2RGB[3] * xyz.y_ + M_XYZ2RGB[6] * xyz.z_;
    rgb.y_ = M_XYZ2RGB[1] * xyz.x_ + M_XYZ2RGB[4] * xyz.y_ + M_XYZ2RGB[7] * xyz.z_;
    rgb.z_ = M_XYZ2RGB[2] * xyz.x_ + M_XYZ2RGB[5] * xyz.y_ + M_XYZ2RGB[8] * xyz.z_;
    return rgb;
};
...
public:
    ...
    SunController()
        ...
        , m_latitude(50.0f)
        , m_month(June)
        , m_eclipticObliquity(ToRadians(23.4f))
        , m_delta(0.0f)
    {
    }
...
private:
    ...
    {
        const float latitude = ToRadians(m_latitude);
        const float hh = _hour * M_PI / 12.0f;
        const float azimuth =
            atan2(sin(hh), cos(hh) * sin(latitude) - tan(m_delta) * cos(latitude));
...
{
    const Vector3 turbidity = {_turbidity, _turbidity, _turbidity};
    for (uint32_t ii = 0; ii < 5; ++ii)
    {
        const Vector3 tmp = ABCDE_t[ii] * turbidity + ABCDE[ii];
...
void ProceduralSky::update_uniform()
{
    ...
    mtl->SetShaderParameter("SunLuminance", GetSunLuminance());
    auto skyLuminance = m_skyLuminanceXYZ->GetValue(m_time);
    mtl->SetShaderParameter("SkyLuminanceXYZ", *(Vector3*)&skyLuminance);
    //mtl->SetShaderParameter("SkyLuminance", GetSkyLuminance());
    auto sunDir = Vector4{ *(Vector3*)&m_sun->m_sunDir, 1.0f/*m_forzone ? 0.5f : 1.0f*/ };
    mtl->SetShaderParameter("SunDirection", sunDir);
    mtl->SetShaderParameter("Parameters", Vector4{ 0.02f, 3.0f, 0.1f, m_time });
    ...
    //mtl->SetShaderParameter("PerezCoeff", v);
    mtl->SetShaderParameter("PerezCoeff0", perezCoeff[0]);
    mtl->SetShaderParameter("PerezCoeff1", perezCoeff[1]);
    mtl->SetShaderParameter("PerezCoeff2", perezCoeff[2]);
    mtl->SetShaderParameter("PerezCoeff3", perezCoeff[3]);
    mtl->SetShaderParameter("PerezCoeff4", perezCoeff[4]);
}
...
Vector3 ProceduralSky::GetSunDirection()
{
    ...
    return { -m_sun->m_sunDir.x_, -m_sun->m_sunDir.y_, -m_sun->m_sunDir.z_ };
}
...
void ProceduralSky::update_environment_texture()
{
    if (!m_scene) {
        ...
        sky->Init(m_verticalCount, m_horizontalCount, (Month)m_sun->m_month, m_time, m_cubemapSize, { ndir.x_, ndir.y_, ndir.z_ });
        m_internalSky = sky;
        ...
        renderTexture->SetSize(m_cubemapSize, m_cubemapSize, TextureFormat::TEX_FORMAT_RGBA8_UNORM, TextureFlag::BindRenderTarget);
