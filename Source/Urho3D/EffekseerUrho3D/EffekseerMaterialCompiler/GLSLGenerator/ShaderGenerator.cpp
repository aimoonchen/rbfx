...
void main()
{
	...
	vec3 worldPos = (modelMatrix * vec4(a_Position, 1.0)).xyz;
...
LAYOUT(0) IN vec3 atPosition;
LAYOUT(1) IN vec4 atColor;
LAYOUT(2) IN vec2 atTexCoord;
)"
	...
	for (int stage = 0; stage < 2; stage++)
	{
		...
		if (useUniformBlock)
		{
			if (useSet_)
			{
				if (stage == 0)
				{
					maincode << "layout(set = 0, binding = 0) uniform VS_ConstantBuffer {" << std::endl;
				}
				else if (stage == 1)
				{
					maincode << "layout(set = 0, binding = 1) uniform PS_ConstantBuffer {" << std::endl;
				}
			}
			else
			{
...
uniform vec4 customData2s[_INSTANCE_COUNT_];
		...
		for (int32_t i = 0; i < actualUniformCount; i++)
		{
            ...
            std::string modifyUniformName("vs_");
            if (stage == 1) {
                modifyUniformName = "fs_";
            }
            modifyUniformName += uniformName;
			ExportUniform(maincode, 4, modifyUniformName.c_str()/*uniformName*/);
		}
        ...
        if (stage == 1) {
            ...
            baseCode = Replace(baseCode, "$LOCALTIME$", "fs_predefined_uniform.w");
        } else if (stage == 0) {
        ...
        for (int32_t i = 0; i < actualUniformCount; i++)
        {
            ...
            auto uniformName = materialFile->GetUniformName(i);
            modifyUniformName += uniformName;
            baseCode = Replace(baseCode, uniformName, modifyUniformName);
        }
		// replace textures
		for (int32_t i = 0; i < actualTextureCount; i++)
		{
