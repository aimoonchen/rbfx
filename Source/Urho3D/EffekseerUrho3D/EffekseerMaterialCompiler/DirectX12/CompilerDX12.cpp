...
CompilerDX12::CompileShaderResultDX12 CompilerDX12::CompileShader(const char* text, const char* fileName,
    const char* target, const std::vector<D3D_SHADER_MACRO>& macro, const CompilerDX12Option& option)
{
    ...
    hr = D3DCompile(text, strlen(text), fileName, macro.size() > 0 ? (D3D_SHADER_MACRO*)&macro[0] : NULL, NULL, "main",
        target, flag, 0, &shader, &error);
...
CompilerDX12::CompilerDX12(const CompilerDX12Option& option)
    : option_(option)
{
}
...
void CompilerDX12::Compile(CompilerResult& result, const char* code, ShaderStageType shaderStage)
{
    ...
    if (compileResult.shader != nullptr)
    {
        ...
        if (compileResult.shader != NULL)
            compileResult.shader->Release();
    }
}
