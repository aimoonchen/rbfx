...
Texture2D<float4> uvDistortionTex : register(t2);
SamplerState uvDistortionTex_sampler : register(s2);
Texture2D<float4> colorTex : register(t0);
SamplerState colorTex_sampler : register(s0);
Texture2D<float4> alphaTex : register(t1);
SamplerState alphaTex_sampler : register(s1);
Texture2D<float4> blendUVDistortionTex : register(t5);
SamplerState blendUVDistortionTex_sampler : register(s5);
Texture2D<float4> blendTex : register(t3);
SamplerState blendTex_sampler : register(s3);
Texture2D<float4> blendAlphaTex : register(t4);
SamplerState blendAlphaTex_sampler : register(s4);
Texture2D<float4> depthTex : register(t6);
SamplerState depthTex_sampler : register(s6);
...
float4 _main(PS_Input Input)
{
    ...
    float4 param_4 = colorTex.Sample(colorTex_sampler, Input.UV_Others.xy + UVOffset);
    ...
    ApplyFlipbook(param_6, colorTex, colorTex_sampler, _433_fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_7, param_8);
    Output = param_6;
    float4 param_9 = alphaTex.Sample(alphaTex_sampler, advancedParam.AlphaUV + UVOffset);
    ...
    float2 BlendUVOffset = UVDistortionOffset(blendUVDistortionTex, blendUVDistortionTex_sampler, param_11, param_12, param_13);
    BlendUVOffset *= _433_fUVDistortionParameter.y;
    float4 param_14 = blendTex.Sample(blendTex_sampler, advancedParam.BlendUV + BlendUVOffset);
    bool param_15 = convertColorSpace;
    float4 BlendTextureColor = ConvertFromSRGBTexture(param_14, param_15);
    float4 param_16 = blendAlphaTex.Sample(blendAlphaTex_sampler, advancedParam.BlendAlphaUV + BlendUVOffset);
